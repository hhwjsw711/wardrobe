...
export function WardrobeImportFlow() {
  ...
  const submitReference = useCallback(async (files) => {
    ...
    try {
      ...
      setNotice({ tone: "complete", text: `${(setup?.modelReferenceCount || 0) + images.length} styling ${(setup?.modelReferenceCount || 0) + images.length === 1 ? "photo" : "photos"} saved`, detail: "These photos will be used together to keep your identity and proportions consistent." });
    ...
    <>
      ...
      {modelRefPanelOpen && (
        <div className="import-popover-backdrop" data-open={true} onMouseDown={(event) => event.target === event.currentTarget && setModelRefPanelOpen(false)}>
          <section className="import-popover" role="dialog" aria-modal="true" aria-labelledby="model-ref-title">
            <header className="import-popover__header"><div><p className="import-popover__eyebrow">Styling photos</p><h2 className="import-popover__title" id="model-ref-title">{referenceCount} {referenceCount === 1 ? "photo" : "photos"} saved</h2></div><button className="import-icon-button" type="button" onClick={() => setModelRefPanelOpen(false)} aria-label="Close styling photos"><X size={20} /></button></header>
      ...
      <div className="import-popover-backdrop" data-open={open} onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
        <section className="import-popover" role="dialog" aria-modal="true" aria-labelledby="import-title">
          ...
          {!jobs.length ? setupRequired ? <div className="import-drop-target import-setup-warning">{missingApiKey ? <><WarningCircle size={30} /><h2>Finish cloud setup</h2><p>Add your OpenAI API key to the Convex environment variables, then redeploy. Your phone never needs the key.</p></> : <><UserFocus size={30} /><h2>Choose photos of yourself</h2><p>Add up to five clear photos from different angles. Wardrobe stores them privately and uses them together for modeled styling.</p><button className="import-button import-button--primary" onClick={() => referenceInputRef.current?.click()}>Choose styling photos</button><p className="import-setup-note">A full-body photo plus a clear face and side angle works well. You can add more later.</p></>}</div> : <div className="import-drop-target"><UploadSimple size={28} /><h2>{notice ? "Ready for clothes" : "Choose or paste images"}</h2><p>{notice?.detail || "The cloud identifies each exact product when the evidence supports it, creates catalog and modeled images, and adds successful pieces automatically."}</p><button className="import-button import-button--primary" disabled={!setup?.ready} onClick={() => { setNotice(null); inputRef.current?.click(); }}>Choose images</button><button className="import-reference-link" disabled={referencesFull} onClick={() => referenceCount > 0 ? (setModelRefPanelOpen(true), setOpen(false)) : referenceInputRef.current?.click()}><UserFocus size={15} /> {referencesFull ? `${referenceCount} styling photos saved` : `Add styling photos · ${referenceCount} saved`}</button></div> : (
            <>
              ...
              <div className="import-actions"><button className="import-button" disabled={referencesFull} onClick={() => { setModelRefPanelOpen(true); setOpen(false); }}><UserFocus size={14} /> {referencesFull ? `${referenceCount} styling photos saved` : `Styling photos · ${referenceCount}`}</button><button className="import-button" onClick={() => inputRef.current?.click()}><Plus size={14} /> Add another</button></div>
            </>
          )}
          {error && <p className="import-status is-error" role="alert">{error}</p>}
        </section>
